...
def main():
    ...
    #training
    theta_new, losses_primal, iterations_primal, steps_primal = training_primal(theta,data,maxiter=100,tol=0,lamb=lamb)
    ...
    #plt.show()
    u_new, losses_dual, iterations_dual, steps_dual = training(u,y,D,maxiter=100,tol=0,lamb=lamb)
    DTu = np.dot(np.transpose(D), u_new)
    theta_u = - y * np.log(DTu/(y - DTu))
    print("It took {} iterations for primal".format(iterations_dual[-1]))
    print("It took {} iterations for dual".format(iterations_primal[-1]))
    steps_dual_theta = [-y * np.log(np.dot(np.transpose(D), i)/(y - np.dot(np.transpose(D), i))) for i in steps_dual]
    calculated_losses_primal = np.array([loss_primal(i, data, lamb) for i in steps_primal])
    calculated_losses_dual = np.array([loss_primal(i, data, lamb) for i in steps_dual_theta])
    f_start = min(np.min(calculated_losses_dual),np.min(calculated_losses_primal)) - 1e-6
    print(f_start)
    print(np.min(calculated_losses_dual))
    print(np.min(calculated_losses_primal))
    print(calculated_losses_dual)
    primal_gap = calculated_losses_primal - f_start
    dual_gap = calculated_losses_dual - f_start
    plt.plot(iterations_primal, np.log(primal_gap), label="Primal criterion gap")
    plt.plot(iterations_dual, np.log(dual_gap), label="Dual criterion gap")
    plt.xlabel("number of iterations")
    plt.ylabel("criterion gap")
    plt.legend()
    plt.show()
    #plots
    plt.plot(range(len(data)), data,"o",label="Data")
    plt.plot(range(len(theta_new)),np.exp(theta_new)/(1+np.exp(theta_new)),"o",label="primal solution")
    plt.plot(range(len(theta_u)),np.exp(theta_u)/(1+np.exp(theta_u)),"o",label="dual solution (Newton's Method)")
